...
export async function GET(request: Request) {
  ...
  if (!user)
    return json({ error: 'Connect your wallet to load saved positions.' }, 401);
  try {
    ...
    if (!q.get('account'))
      return json({ accounts: await store.list(user, wallet) });
    ...
    return json(
      {
        error:
          'Saved activity could not be loaded. Your wallet and position are unchanged.',
      },
      503,
    );
  }
...
export async function POST(request: Request) {
  ...
  if (!user)
    return json(
      { error: 'Connect your wallet to save position activity.' },
      401,
    );
  if (!sameOrigin(request))
    return json({ error: 'This request must come from Freestock.' }, 403);
  ...
  try {
    ...
    const body = JSON.parse(new TextDecoder().decode(bytes)) as Record<
      ...
      unknown
    >;
    requireWalletOwner(user, body.owner);
    ...
    return await withRpcReads(async () => {
      if (body.action === 'remember')
        return json({
          account: await service.register(user, wallet, deployment),
        });
      if (body.action === 'sync')
        return json(await service.sync(user, wallet, deployment));
