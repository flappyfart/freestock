...
export function HomePools() {
  const [asset, setAsset] = useState<"usdg" | "stocks">("usdg");
    ...
    <section className="home-pools" aria-labelledby="home-pools-title">
      <div className="home-pools-heading">
        <div>
          ...
          <p>Explore USDG and Stock Token markets with real data from Morpho.</p>
        </div>
        {asset === "usdg" && (
      ...
      <fieldset className="home-pools-asset-switch" aria-label="Lending asset">
        <button type="button" aria-pressed={asset === "usdg"} onClick={() => setAsset("usdg")}>
          USDG lending
        </button>
          ...
          {catalogue && (
            <>
              ...
              {catalogue.markets.length === 0 && (
                <p className="home-pools-placeholder">
                  No tracked markets were returned by the feed.
                </p>
              )}
              <div className="home-pools-footer">
                <div>
                  <p>
                    Tracked subset, not every pool on Robinhood Chain. These markets are for
                    discovery; Freestock currently supports the USDG vault above. Vault assets
                    overlap with market supply and should not be added together.
                  </p>
                  <p>
                    Historical rates can change or turn negative. Rewards excluded; vault fees are
                    reflected in its net APY. Data fetched{" "}
                    <time dateTime={catalogue.asOf}>
                      {new Date(catalogue.asOf).toLocaleString()}
                    </time>
                    . Refreshes each minute while this tab is visible; source responses may be
