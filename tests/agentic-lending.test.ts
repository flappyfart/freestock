...
const check = (a: AgentAccount | null, plan: AgentPlan = defaultAgentPlan, q?: AgentQuote) =>
...
void test("quotes preserve their exact spending amount as surplus changes", () => {
  ...
  assert.equal(check(account(), defaultAgentPlan, quote(defaultAgentPlan, "999999")).code, "quote");
});
void test("quote requests and recommendations honor the 100 USDG pilot action limit", () => {
  const large = account({
    assetValue: "210000002",
    availableGains: "200000002",
    spendableWithRoundingBuffer: "200000000",
  });
  assert.equal(agentPurchaseAmount(large), 100_000_000n);
  assert.equal(check(large, defaultAgentPlan, quote(defaultAgentPlan, "100000000")).code, "review");
  assert.equal(check(large, defaultAgentPlan, quote(defaultAgentPlan, "100000001")).code, "quote");
});
...
void test("losses recover first and retention requires no transaction or quote", () => {
  assert.equal(
    check(account({ assetValue: "9000000", availableGains: "0", spendableWithRoundingBuffer: "0" }))
      .code,
    "loss",
  );
...
void test("pause and pending actions prevent actionable advice", () => {
  assert.equal(check(account(), { ...defaultAgentPlan, paused: true }, quote()).code, "paused");
...
void test("cost at ETH limit qualifies for review, above limit waits; no USDG conversion is inferred", () => {
  ...
  q.prepared.estimatedGasCostWei = (BigInt(defaultAgentPlan.maximumGasWei) + 1n).toString();
...
void test("invalid weights, unsupported stocks and malformed persisted settings are rejected", () => {
  for (const allocations of [
    ...
    assert.throws(() => validateAgentPlan({ ...defaultAgentPlan, allocations }));
  assert.throws(() => validateAgentPlan({ ...defaultAgentPlan, maximumGasWei: "-1" }));
  assert.throws(() => validateAgentPlan({ ...defaultAgentPlan, minimumGains: "0" }));
});
void test("manual handoff binds provider scope, wallet, account, deployment, amount and expiry", () => {
  ...
  for (const changes of [
    ...
    assert.throws(() =>
      validateAgentIntent({ ...intent, ...changes }, intent.scope, account(), owner, now),
    );
});
