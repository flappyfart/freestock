...
import {
  ...
  clearJournal,
  submitJournaled,
} from '../lib/live/wallet-journal.ts';
...
void test('live baskets allocate each micro unit once and reject unsafe selections', () => {
  const allocations = PILOT_STOCKS.map((s) => ({
    symbol: s.symbol,
    weightBps: 2000,
  }));
...
void test('approval is bound to the exact reviewed account and amount', () => {
  ...
  p.transaction.data = tokenAbi.encodeFunctionData('approve', [
    account,
    10_000_000n,
  ]);
...
void test('ambiguous post-send errors prevent retries; explicit rejection releases the journal', async () => {
  ...
  await assert.rejects(
    submitJournaled(unknown, plan(), owner, null, () => true),
  );
  ...
  await assert.rejects(
    submitJournaled(
      provider((m) => {
        if (m === 'eth_sendTransaction')
          throw Object.assign(Error('Rejected'), { code: 4001 });
      }),
      plan(),
      owner,
      null,
...
void test('large exact approvals and deposits work without permitting unlimited allowance', () => {
  const p = plan();
  const large = 250_000_000n;
  Object.assign(p, { action: 'approve', account, assets: large.toString() });
  p.transaction.to = USDG;
  p.transaction.data = tokenAbi.encodeFunctionData('approve', [account, large]);
  assert.doesNotThrow(() => validatePrepared(p, owner, account));
  p.assets = ((1n << 256n) - 1n).toString();
  p.transaction.data = tokenAbi.encodeFunctionData('approve', [
    account,
    BigInt(p.assets),
  ]);
  assert.throws(() => validatePrepared(p, owner, account));
  Object.assign(p, { action: 'deposit', assets: large.toString() });
  p.transaction.to = account;
  p.transaction.data = accountAbi.encodeFunctionData('deposit', [large, 1n]);
  assert.doesNotThrow(() => validatePrepared(p, owner, account));
});
