...
// These disposable wallet profiles are used only against an isolated local Worker.
// The hostname guard and redirect refusal must remain intact.
...
async function request(path, options = {}) {
  ...
  if (identity) {
    if (!sessions.has(identity.id))
      sessions.set(identity.id, localWalletSession(ORIGIN));
    headers.set('cookie', (await sessions.get(identity.id)).cookie);
  }
...
async function readAccount(identity = alice) {
  const data = expectStatus(
    await request('/api/earn/account', { user: identity }),
    200,
  );
...
async function command(identity, body, commandKey = key(body.type)) {
  for (let attempt = 0; attempt < 5; attempt++) {
    let result;
    try {
      result = await request('/api/earn/commands', {
        user: identity,
        body,
        key: commandKey,
      });
    ...
    assert.ok(
      Number.isSafeInteger(data.receipt.version) && data.receipt.version > 0,
    );
  ...
  throw new Error(
    `Could not confirm command ${commandKey}; retry this same key.`,
  );
}
...
async function main() {
  ...
  await check(
    'anonymous and incomplete identities cannot access accounts or mutate',
    async () => {
  ...
  await check(
    'duplicate deposit applies once and key mismatch rejects',
    async () => {
      const k = key('same');
  ...
  await check(
    'owner separation and query spoofing preserve separate accounts',
    async () => {
  ...
  await check('concurrent same-key requests apply exactly once', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 5 }, () =>
        command(alice, { ...open, amount: 100e6 }, k),
      ),
    );
  ...
  await check(
    'concurrent distinct requests preserve every deposit',
    async () => {
      const before = await readAccount();
      await Promise.all(
        [10, 20, 30, 40].map((n) =>
          command(alice, { ...open, amount: n * 1e6 }),
        ),
      );
      const after = await readAccount();
  ...
  await check(
    'auto-conversion and compounding save together and reflect in reads',
    async () => {
  ...
  await check(
    'closing remains idempotent and does not erase holdings',
    async () => {
      const k = key('close');
      const first = await command(
        alice,
        { type: 'close', positionId: 'p1' },
        k,
      );
      ...
      await rejected(
        { type: 'advance', positionId: 'p1', days: 1, shockPct: 0 },
        422,
      );
    },
  );
  await check(
    'LP loss scenario defers both compounding and conversion',
    async () => {
  ...
  await check(
    ...
    async () => {
      const m = expectStatus(
        await request('/api/markets', { user: null }),
        200,
      );
