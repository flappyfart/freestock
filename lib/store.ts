...
import { applyCommand, digest, initialState, type Command, type State, RuleError } from "./engine";
export function db() {
...
export async function account(owner: string) {
  ...
  let row = await database
    .prepare("SELECT state, version FROM accounts WHERE owner = ?")
    .bind(owner)
    .first<{ state: string; version: number }>();
  if (!row) {
    await database
      .prepare(
        "INSERT INTO accounts(owner,state,version,updated_at) VALUES(?,?,0,?) ON CONFLICT(owner) DO NOTHING",
      )
      .bind(owner, JSON.stringify(initialState()), new Date().toISOString())
      .run();
    row = await database
      .prepare("SELECT state, version FROM accounts WHERE owner = ?")
      .bind(owner)
  ...
  return { state: JSON.parse(row.state) as State, version: row.version };
}
export async function execute(owner: string, key: string, command: Command) {
  const database = db(),
  ...
  const receipt = async () => {
    const row = await database
      .prepare("SELECT fingerprint, receipt FROM commands WHERE owner = ? AND key = ?")
      .bind(owner, key)
      .first<{ fingerprint: string; receipt: string }>();
    if (!row) return null;
    if (row.fingerprint !== fingerprint)
      throw new RuleError("This request ID was already used for a different action.");
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    let next: State;
    try {
      next = await applyCommand(current.state, command);
    } catch (error) {
      const prior = await receipt();
      if (prior) return { ...(await account(owner)), receipt: prior, replayed: true };
      throw error;
    ...
    try {
      ...
      await database.batch([
        database
          .prepare(
            "UPDATE accounts SET state = ?, version = version + 1, last_command = ?, updated_at = ? WHERE owner = ? AND version = ?",
          )
          .bind(JSON.stringify(next), key, now, owner, current.version),
        database
          .prepare(
            "INSERT INTO commands(owner,key,fingerprint,receipt,created_at) SELECT ?,?,?,?,? WHERE EXISTS (SELECT 1 FROM accounts WHERE owner = ? AND version = ? AND last_command = ?)",
          )
          .bind(
            owner,
            key,
