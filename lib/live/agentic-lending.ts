...
const integer = (value: unknown) => {
  ...
  if (n > MaxUint256) throw Error('Amount is outside the token range.');
  return n;
};
...
export function validateAgentPlan(input: unknown): AgentPlan {
  if (!input || typeof input !== 'object')
    throw Error('Choose a lending plan.');
  ...
  if (
    ...
    throw Error(
      'Enter a positive conversion minimum and a gas limit up to 1 ETH.',
    );
...
export function accountIsFresh(
  ...
  try {
    const budget = positionBudget(
      integer(account.principal),
      integer(account.assetValue),
    );
    return (
...
export function evaluateLending(input: {
  ...
  const budget = positionBudget(
    BigInt(account.principal),
    BigInt(account.assetValue),
  );
  ...
  try {
    ...
    if (
      ...
      throw Error();
    if (
      !same(p.owner, input.owner) ||
      !p.account ||
      !same(p.account, account.account)
    )
      throw Error();
    if (
      !Number.isFinite(Date.parse(p.expiresAt)) ||
...
export function validateAgentIntent(
  ...
  if (
    ...
    throw Error(
      'This recommendation no longer matches the active position. Check the plan again.',
    );
