...
export function validatePrepared(
  ...
  if (plan.action === 'approve') {
    ...
    if (
      ...
      throw Error(
        'Token approval does not match the reviewed amount and account.',
      );
    return;
  }
  if (!same(tx.to, verifiedAccount))
    throw Error('The transaction targets another account.');
  ...
  if (['deposit', 'compound'].includes(plan.action)) {
    ...
    if (
      ...
      throw Error(
        'The stock purchase differs from the reviewed route or limits.',
      );
    ...
    for (const [i, p] of purchases.entries()) {
      ...
      if (
        ...
        throw Error(
          'A basket leg differs from its reviewed amount, stock or minimum.',
        );
      seen.add(stock.symbol);
      total += a[2][i];
    }
    if (total !== assets)
      throw Error('Basket spending does not match the reviewed total.');
...
export async function submitPrepared(
  ...
  if (
    ...
    !same(accounts[0], owner) ||
    network !== '0x1237'
  )
    throw Error(
      'The wallet or network changed. Reconnect and refresh this preview.',
    );
  validatePrepared(plan, owner, account);
  beforeSend?.();
