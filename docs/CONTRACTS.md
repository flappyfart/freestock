...
## Accounting and account versions
...
The V2 constructor accepts any positive cap; canonical Freestock deployments pass `MaxUint256`, removing the product’s deposit limit. A deposit still checks `principal + amount <= depositCap` using integer accounting. Legacy V1 accounts were deployed with 100 USDG and cannot be upgraded. Their old cap applies against the principal baseline when adding deposits; it is not a lifetime contribution limit or a maximum portfolio value. Existing V1 positions remain restorable and withdrawable. New V2 creation never migrates funds or grants a token allowance automatically.
...
Direct creation accepts ordinary key-controlled wallets and exact EIP-7702 delegated EOAs. The wallet still sends an ordinary contract-creation transaction; Freestock does not add a delegation or use a relayer. Receipt verification still requires the exact sender, nonce-derived address, constructor data, runtime and immutable configuration. Contract-wallet wrappers and batched creation are not supported. See [EIP-7702 transaction origination](https://eips.ethereum.org/EIPS/eip-7702#transaction-origination).
